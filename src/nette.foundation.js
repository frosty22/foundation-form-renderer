 ...
 *
 * Copyright (c) 2013, Ledvinka Vít - for Foundation
 */
...
Nette.addError = function(elem, message) {
	if (message) {
		var wrapp = $(elem).parent();
		wrapp.addClass('error');

		if (wrapp.find("small.error").length) {
			wrapp.find("small.error").append('<br />' + message);
		} else {
			wrapp.append('<small class="error">' + message + '</small>');
		}
...
/**
 * Clean errors on element.
 */
Nette.cleanErrors = function(elem) {
	$(elem).parent().find('small.error').remove();
